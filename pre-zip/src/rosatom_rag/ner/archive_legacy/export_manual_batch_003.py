...
EXISTING_CORPUS_PATH = PROJECT_ROOT / "data" / "ner" / "labeled" / "manual_corpus_v2.jsonl"
OUTPUT_PATH = PROJECT_ROOT / "data" / "ner" / "labeled" / "manual_batch_003.json"
...
def score_record(text: str, entities: list[dict]) -> int:
    ...
    score += label_counts.get("DOC_NUM", 0)
...
def main():
    ...
    existing_corpus = load_jsonl(EXISTING_CORPUS_PATH)

    existing_ids = {make_record_id(r) for r in existing_corpus}
    ...
    print("Пропущено уже размеченных:", skipped_existing)
    print("Пропущено слишком коротких:", skipped_short)
