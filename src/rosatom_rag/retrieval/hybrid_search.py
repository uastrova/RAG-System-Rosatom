...
# FAISS + BM25 + логика для поиска таблиц + реранкер 



# нормализация: нижний регистр, убираем лишние пробелы, в именительный падеж слово "таблица"
...
# ищем ближайшие по смыслу чанки 
...
def exact_table_reference_search(question: str, k: int = 10):
    ...
    # нормализуем вопрос и вытаскиваем ссылки на таблицы из вопроса 
    ...
    for doc in docs:
        # нормализуем каждый чанк так же как и вопрос
        ...
        # совпадение найдено -> чанку начисляется score
        score = float(len(matched_refs))

        # если найденный чанк сам является таблицей
...
# объединяем и считаем общий скор hybrid score
